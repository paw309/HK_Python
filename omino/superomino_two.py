...
def stitch_pair(pool: List[Dict], max_attempts: int = 50000):
                    ...
                    for reflB in [False, True]:
                        ...
                        for dx in range(0, max_dx + 1):
                            for dy in range(0, max_dy + 1):
                                ...
                                # four stitch modes: A.end→B.start, A.end→B.end, A.start→B.start, A.start→B.end
                                candidates = [
    ...
    raise RuntimeError("Failed to stitch pair within attempt limit")
...
def generate_stitched_pair(base_dir: str, n: int):
    pool = load_pool(base_dir, n)
    cells, tour, (idA, idB) = stitch_pair(pool)
...
def main():
    ...
    for n in range(10, 17):
        try:
            cells, tour, idA, idB = generate_stitched_pair(base_dir, n)
        except Exception as e:
            print(f"n={n}: failed ({e})")
            continue

        print(f"\nn={n}: stitched {idA} + {idB}, tour length={len(tour)}")
        print(ascii_visualize(cells, tour))

        svg = svg_visualize_tour(tour)
        out_path = os.path.join(base_dir, f"stitched_{n}_{idA}_{idB}.svg")
