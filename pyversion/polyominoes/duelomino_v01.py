...
Two-player competitive polyomino game.
Players take alternating turns to find more polyomino units than their opponent.
"""
...
for _p in (BASE_DIR, SHAREDLIB, GAME_DIR, POLYOMINOES_DIR):
    if _p not in sys.path:
        sys.path.insert(0, _p)

from gameboard import BoardModel, BoardRenderer

from pyversion.polyominoes.duelomino_controller import (
    DuelominoController,
    BOARD_MIN, BOARD_MAX, BOARD_DEFAULT, FPS,
    SHAPES_CHOICES, DENSITY_CHOICES, COLORS_CHOICES, MAX_CLOCK_MINUTES,
...
def main() -> None:
    ...
    pygame.display.set_caption("Duelomino")
    ...
    menu_items = [
        ...
        ("shapes", SHAPES_CHOICES[:], 4),  # idx 4 -> pentomino
        ("density", DENSITY_CHOICES[:], 1),  # idx 1 -> medium
        ("colors", COLORS_CHOICES[:], 0),  # idx 0 -> unique
        ("first move", PLAYER_ONE_CHOICES[:], 0),  # idx 0 -> human
        ("level", OPPONENT_LEVEL_CHOICES[:], 0),  # idx 0 -> beginner
        ("clock", [0] + list(range(60, MAX_CLOCK_MINUTES * 60 + 1, 60)), 0),  # 0 = no limit
    ]
    ...
    # --- controller ---
    controller = DuelominoController(
