"""
Script to construct 18-square polyominoes (icosominoes) that support knight's tours
by joining two 8-square knight's-tour-supporting polyominoes (decominoes).
...
from tours_decomino import DECOMINO_TOURS
...
def generate_icosomino_tours(decomino_tours: Dict[str, Path],
                                max_results: int = None) -> Tuple[Dict[str, Path], Dict[str, Path]]:
    """
    Generate all unique icosomino tours by combining pairs of decomino tours.
    ...
    decomino_keys = list(decomino_tours.keys())
    total_pairs = len(decomino_keys) * len(decomino_keys)
    processed = 0

    print(f"Processing {total_pairs} pairs of decominoes...")

    # Try all pairs (including same decomino with itself in different orientations)
    for key1 in decomino_keys:
        path1_original = decomino_tours[key1]

        for key2 in decomino_keys:
            path2_original = decomino_tours[key2]
...
def write_tours_file(tours: Dict[str, Path], filename: str, var_name: str):
    """Write tours to a Python file in the same format as tours_decomino.py"""
...
def main():
    """Main function to generate icosomino tours."""
    import os

    print("=" * 80)
    print("Icosomino Tour Generator")
    print("Constructing 29-square polyominoes from decomino pairs")
    print("=" * 80)
    print()

    # Load decomino tours
    print(f"Loaded {len(DECOMINO_TOURS)} decomino tours")
    print()

    # Generate icosomino tours
    # Set max_results to limit output during development/z_testing
    # Remove or set to None for full generation
    all_tours, closed_tours = generate_icosomino_tours(
        DECOMINO_TOURS,
    ...
    all_tours_file = os.path.join(output_dir, "generated_icosomino_tours.py")
    closed_tours_file = os.path.join(output_dir, "generated_icosomino_closed_tours.py")

    write_tours_file(all_tours, all_tours_file, "ICOSOMINO_TOURS")
    write_tours_file(closed_tours, closed_tours_file, "ICOSOMINO_CLOSED_TOURS")
