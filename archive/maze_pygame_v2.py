...
# ---- Pygame GUI ----
def draw_board(screen, board_w, board_h, knight_pos, target, obstacles, revealed, move_numbers):
...
def main():
    ...
    while running:
            ...
            if event.type == pygame.QUIT:
                ...
                if MARGIN <= mx < MARGIN + board_w*SQUARE_SIZE and MARGIN <= my < MARGIN + board_h*SQUARE_SIZE:
                    ...
                    if (r, c) in squares_one_move_away(board_h, board_w, knight_pos):
                        if (r, c) in visited:
                            # Prevent backtracking to previously visited squares
                            continue
                        if not started:
                            started = True
        ...
        draw_board(screen, board_w, board_h, knight_pos, target, obstacles, revealed, move_numbers)
        # Info
        if started:
