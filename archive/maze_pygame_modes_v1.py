...
def find_knight_path_exact_x(m, n, start, end, x, y):
    tries = 0
    while True:
        visited = [[False for _ in range(n)] for _ in range(m)]
        visited[start[0]][start[1]] = True
        path = [start]
        res = _backtrack_knight_path(m, n, start[0], start[1], end, x, 0, path, visited)
        if res:
            if y == 2:
...
def _backtrack_knight_path(m, n, r, c, end, x, depth, path, visited):
    if depth > x:
        return None
    ...
    for dr, dc in moves:
        ...
        if is_valid(nr, nc, m, n) and not visited[nr][nc]:
            visited[nr][nc] = True
            path.append((nr, nc))
            result = _backtrack_knight_path(m, n, nr, nc, end, x, depth + 1, path, visited)
            if result:
                return result
            path.pop()
            visited[nr][nc] = False
...
def generate_maze(settings):
    ...
    while True:
        ...
        if start == target:
            continue
        y, _ = min_moves_and_dist_matrix(m, n, start, target)
        if y < 2:
            continue
        min_x = m // 2 + y
        max_x = m + y
        ...
        for x in possible_x:
            maze_path = find_knight_path_exact_x(m, n, start, target, x, y)
            if maze_path:
                break
        else:
            continue
    ...
    for sq in shortest_path_layer:
        if sq not in maze_path_set:
            obstacles.add(sq)
    extra_obstacles = set()
    remaining_squares = {(r, c) for r in range(m) for c in range(n)} - maze_path_set - obstacles - {start, target}
    ...
    return {
        ...
        "timer_length": settings.get("timer_length", 5*60)  # in seconds
    }
...
def pygame_custom_settings():
    ...
    # If custom timer, ask for minutes
    if settings["timer_type"] == "countdown":
        selected = 4  # default to 5 minutes
...
def draw_board(screen, board_w, board_h, knight_pos, target, obstacles, revealed, move_numbers, show_obstacles, maze_path=[], endgame=False):
    ...
    for r in range(board_h):
        for c in range(board_w):
            ...
            # Endgame: maze path squares are purple
            ...
            # Always draw knight and target, regardless of board parity/dimensions
            if (r, c) == target:
...
def main():
    ...
    while running:
        ...
        screen.fill((50, 50, 50))
        # End of game: show maze path in purple and all obstacles revealed
        if endgame:
            ...
            # In invisible mode, show all flashed obstacles for 5 seconds
        ...
        if endgame:
            if won:
                screen.blit(font.render("     You solved the Knight's Maze!", True, (255,255,0)), (MARGIN, MARGIN - 36))
            elif time_up:
